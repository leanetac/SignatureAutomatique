...
class TunnelManagementHttpClient {
    ...
    async createTunnel(tunnel, options) {
        ...
        for (let i = 0; i <= createNameRetries; i++) {
            try {
                const result = (await this.sendTunnelRequest('PUT', tunnel, manageAccessTokenScope, undefined, undefined, options, this.convertTunnelForRequest(tunnel), undefined, true));
                preserveAccessTokens(tunnel, result);
                parseTunnelDates(result);
                return result;
            }
        ...
        const result2 = (await this.sendTunnelRequest('PUT', tunnel, manageAccessTokenScope, undefined, undefined, options, this.convertTunnelForRequest(tunnel)));
        preserveAccessTokens(tunnel, result2);
        parseTunnelDates(result2);
        return result2;
    }
    async createOrUpdateTunnel(tunnel, options) {
        const tunnelId = tunnel.tunnelId;
        const idGenerated = tunnelId === undefined || tunnelId === null || tunnelId === '';
        if (idGenerated) {
            tunnel.tunnelId = idGeneration_1.IdGeneration.generateTunnelId();
        ...
        const result2 = (await this.sendTunnelRequest('PUT', tunnel, manageAccessTokenScope, undefined, "forceCreate=true", options, this.convertTunnelForRequest(tunnel), undefined, true));
        preserveAccessTokens(tunnel, result2);
        parseTunnelDates(result2);
        return result2;
    }
    ...
    async updateTunnelEndpoint(tunnel, endpoint, options) {
        ...
        const result = (await this.sendTunnelRequest('PUT', tunnel, hostAccessTokenScope, path, "connectionMode=" + endpoint.connectionMode, options, endpoint));
        if (tunnel.endpoints) {
    ...
    async deleteTunnelEndpoints(tunnel, id, options) {
    ...
    async createOrUpdateTunnelPort(tunnel, tunnelPort, options) {
        tunnelPort = this.convertTunnelPortForRequest(tunnel, tunnelPort);
        const path = `${portsApiSubPath}/${tunnelPort.portNumber}`;
        const result = (await this.sendTunnelRequest('PUT', tunnel, managePortsAccessTokenScopes, path, undefined, options, tunnelPort));
        tunnel.ports = tunnel.ports || [];
     ...
     */
    async sendTunnelRequest(method, tunnel, accessTokenScopes, path, query, options, body, allowNotFound, isCreate = false) {
